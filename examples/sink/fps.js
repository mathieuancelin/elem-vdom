  ...
  function Bubbles() {
    ...
    if (!bubbles) {
      ...
      for (let i = 0; i < bubblesCount; i++) {
        bubbles.push({ x: rnd(600), y: rnd(600), r: rnd(50), speed: baseSpeed, incrementX: baseSpeed, incrementY: baseSpeed, color: colors[parseInt(rnd(6), 10) - 1] });
      }
    }
    for (let i in bubbles) {
      let bubble = bubbles[i];
      bubble.speed = baseSpeed + (Math.random() * 5);
